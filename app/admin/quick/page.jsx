...
const GROUPS = [
  {
    title: 'Contact',
    fields: [
      { label: 'Booking / contact email', path: 'site.email', type: 'email', hint: 'Where your Book Me and Email buttons send people.' },
    ],
  },
  {
    title: 'Social links',
...
export default function Quick() {
  ...
  function update(path, val) {
    setContent((c) => { const copy = JSON.parse(JSON.stringify(c)); setIn(copy, path, val); return copy; });
  }
  async function save() {
    ...
    <div className="q">
      ...
      {status === 'login' && (
        <div className="qcenter">
          <div className="qcard" style={{ textAlign: 'center', maxWidth: 380 }}>
            <h2>Please log in</h2>
            <p className="qmuted">You need to be logged in to edit settings.</p>
      ...
      {status === 'in' && content && (
        <div className="qwrap">
          ...
          <div className="qfoot">
            <button className="qbtn save big" disabled={busy || !dirty} onClick={save}>{busy ? 'Saving...' : dirty ? 'Save changes' : 'All saved'}</button>
          </div>
        </div>
      )}
    </div>
...
.qbar{position:sticky;top:0;z-index:10;background:rgba(239,233,221,.92);backdrop-filter:blur(8px);border-bottom:1px solid var(--line);display:flex;align-items:center;justify-content:space-between;padding:13px 22px}
.qbrand{font-family:'Anton',sans-serif;font-size:22px;letter-spacing:.04em}
...
.qbtn{border:1.5px solid var(--ink);background:transparent;color:var(--ink);border-radius:100px;padding:9px 18px;font-weight:700;font-size:13px;cursor:pointer;text-decoration:none}
...
.qcard h3{font-family:'Anton',sans-serif;font-size:19px;text-transform:uppercase;letter-spacing:.02em;margin-bottom:14px;color:var(--ink)}
